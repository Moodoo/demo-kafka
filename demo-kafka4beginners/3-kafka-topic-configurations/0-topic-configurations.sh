...
port 19092 from inside docker container
# list topics
kafka-topics.sh --bootstrap-server localhost:19092 --list

# create a topic that we'll configure
kafka-topics.sh --bootstrap-server localhost:19092 --create --topic configured-topic --partitions 3 --replication-factor 1
...
kafka-configs.sh --bootstrap-server localhost:19092 --entity-type topics --entity-name configured-topic --describe

# add a config to our topic
kafka-configs.sh --bootstrap-server localhost:19092 --entity-type topics --entity-name configured-topic --add-config min.insync.replicas=2 --alter
